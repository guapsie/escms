...
if (!defined('ESCMS_VERSION')) exit;

$method = $_SERVER['REQUEST_METHOD'] ?? 'GET';
$input = json_decode(file_get_contents('php://input'), true) ?? [];

$send_json = function ($data, $code = 200) {
...
$action = str_replace('api/addons/ai-copilot/', '', $route);
...
switch ($action) {
    ...
    case 'generate':
        ...
        $atoms_dir = dirname(__DIR__, 2) . '/atoms';
        $available_atoms = [];
        if (is_dir($atoms_dir)) {
        ...
        $media_dir = dirname(__DIR__, 2) . '/media';
        $available_media = [];
        if (is_dir($media_dir)) {
