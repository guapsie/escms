...
export class EscmsParser {
    ...
    static RELEVANT_ATTRS = ['src', 'href', 'alt', 'title', 'target', 'id', 'ref', 'aria-label', 'placeholder', 'type', 'name', 'value', 'data-escms-mesh', 'data-escms-anim', 'data-escms-layout'];
    ...
    static jsonToDom(jsonNode) {
        ...
        if (jsonNode.atom) {
            ...
            let atomDef = window.escmsAtoms?.[atomName];
            
            if (!atomDef && window.escmsAtomCategories) {
                for (const cat of window.escmsAtomCategories) {
                    const found = cat.atoms?.find(a => a.name === atomName || a.atom === atomName || a.id === atomName);
                    if (found) {
                        atomDef = found;
                        break;
                    }
